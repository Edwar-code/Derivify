...
interface DerivConnectionCardProps {
    // Pass the current status from the server component
...
export function DerivConnectionCard({ initialPoaStatus = 'none' }: DerivConnectionCardProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!response.ok) {
            ...
            // In Next.js 13+ App Router, you revalidate server-side data by refreshing the page.
            router.refresh();
        }
    };
                    ...
                    <div className="space-y-2">
                        ...
                        />
                         <p className="text-xs text-muted-foreground pt-1">
                            Your token is encrypted and used only for status verification.
                        </p>
                    </div>
