...
export default function MainLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen w-full flex-col bg-background text-foreground">
      ...
      {/* --- UPDATED FOOTER --- */}
      <footer className="sticky bottom-0 flex h-16 items-center gap-4 border-t bg-card px-4 md:px-6 z-50 text-muted-foreground">
        <div className="flex items-center gap-4 text-sm flex-1">
          ...
          <div className="flex items-center gap-4">
            ...
            >
              <Sun className={`h-4 w-4 absolute ${theme === 'light' ? 'block' : 'hidden'}`} />
              <Moon className={`h-4 w-4 absolute ${theme === 'dark' ? 'block' : 'hidden'}`} />
            ...
            {/* Privacy Policy Link */}
            <Link href="/privacy-policy" aria-label="Privacy Policy" className="text-muted-foreground hover:text-primary">
              Privacy Policy
            </Link>
          </div>
        </div>
